'''
For low-frequency signals, restoration quality is superior under low noise assumptions, so Gaussian sigma is set low.
Reference: Jiralerspong et al. (2025). "Shaping Inductive Bias in Diffusion Models through Frequency-Based Noise Control." arXiv:2502.10236
'''

from Utilities.AncillaryFunctions64 import compute_snr
...
class LearnedSchedule(tf.keras.Model):
    ...
    def __init__(self, GammaMin=None, GammaMax=None, **kwargs):
        ...
        self.b = tf.Variable(-1.0, trainable=True, dtype=tf.float64, name="b")
        self.w = tf.Variable(-1.0, trainable=True, dtype=tf.float64, name="w")
...
class Block(tf.keras.Model):
    ...
    def call(self, inputs, embedding, condition):
        ...
        if not self.last:
            residual = (self.proj_res(x) + inputs) / tf.cast(tf.math.sqrt(2.0), dtype=tf.float64)
        else:
            residual = None
...
class WaveNet(tf.keras.Model):
    ...
    def call(self, signal, timestep, condition):
        ...
        out = tf.add_n(skip_connections) / tf.math.sqrt(tf.cast(len(self.blocks), dtype=tf.float64))
...
class VDM(tf.keras.Model):
    ...
    def compute_loss(self, x, condition, noise=None):
        ...
        recons_loss = self.log_probs_x_z0(x, z_0=None, min_sigma=self.cfg['SigmaMin'], 
                                          RecLossType=self.cfg['RecLossType'])  # shape [B]
...
def VDiffWAVE_Restoration(Model, DiffusedSignals, Condition, GenSteps=10, StepInterval=1, GenBatchSize=100, Noise=None, GPU=True):
    ...
    Args:
        Model: The diffusion model used for sampling (VDM or similar).
        DiffusedSignals: [B, T, C] diffused signals to restore (initial latent).
        Condition: [B, cond_dim] conditioning inputs.
        GenSteps: Integer, number of reverse-diffusion steps to perform.
        GenBatchSize: Integer, how many samples to process at once.
        Noise: Optional noise (not used in this example, but can be integrated if needed).
    ...
    with tf.device(device):
        # Assume Model.signal_shape = (T, C)
        # and DiffusedSignals has shape [B, T, C]
        B = tf.shape(DiffusedSignals)[0]  # total number of samples
        T, C = Model.signal_shape

        # We'll keep 'Sample' updated as we go from t down to s
        ...
        for i in pbar:
            ...
            # We'll process each sub-batch of size GenBatchSize
            start_idx = 0
            sub_outputs = []  # To store each sub-batch's updated sample

            start_time = time.time()
            while start_idx < B:
                end_idx = tf.minimum(start_idx + GenBatchSize, B)

                # Extract sub-batch of size [sub_B, T, C]
                sub_sample = Sample[start_idx:end_idx]
                # Corresponding conditions [sub_B, cond_dim]
                sub_condition = Condition[start_idx:end_idx]

                sub_B = end_idx - start_idx

                # Make a tensor of timesteps for this sub-batch
                t_batch = tf.cast(tf.ones((sub_B,), dtype=tf.float64) * t_, tf.int64)
                s_batch = tf.cast(tf.ones((sub_B,), dtype=tf.float64) * s_, tf.int64)

                # Call model on the sub-batch
                sub_sample_updated = Model.sample_p_s_t(z=sub_sample, t=t_batch, s=s_batch, condition=sub_condition, clip_samples=False)
                sub_outputs.append(sub_sample_updated)
                start_idx = end_idx  # move to next sub-batch

            # Concatenate sub-batch results -> [B, T, C]
            Sample = tf.concat(sub_outputs, axis=0)
            print('Time elapsed:',  time.time() - start_time)
            
            # Update progress bar
            pbar.set_postfix({"Current Step": i + 1})
